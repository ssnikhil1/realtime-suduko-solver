...
##contour detection(largest four point contour detection)
...
##After finding contour points ,i dont know which one is top left corner ,  bottom right,top right,bottom left ,so that i ordering the points in particular order (i.e top left,top,right,bottom left,bottom right)
...
##transforming axis of sudoku image
...
##split transformed image into 81 small images for prediction
def split_image(img):
...
##predicting all 81 images(i.e 0 if blank and predicted digit if not blank) and store it into a list result
...
##displaying numbers on image in particular area
...
##solvig suduko using backtracking
...
while True:
    ret, fram = cap.read()
    fram=cv2.resize(fram,(800,600))
    img1=fram.copy()
    print(fram.shape)
    imgBlank = np.zeros((800, 600, 3), np.uint8)
    ...
    if biggest.size != 0:
        img_contour = cv2.drawContours(img_contour, biggest, -1, (0, 0, 255), 10)
        new_points=order_points(biggest)
        wrap_image= transform_image(new_points,img1)
        ...
        t_matrix = cv2.getPerspectiveTransform(pts1, pts2)
        dst	=cv2.warpPerspective(src=imgSolvedDigits,M=t_matrix,dsize=(800,600), flags=cv2.WARP_INVERSE_MAP)

        inv_perspective = cv2.addWeighted(dst, 1, fram, 0.5, 1)
        cv2.imshow('final_output', inv_perspective)
